    ...
    if (generatedPlan) {
        return (
            <div className="fixed top-[56px] left-0 right-0 bottom-0 z-40 bg-slate-900 flex flex-col animate-in fade-in duration-300">
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-zinc-400 mt-0.5">{generatedPlan.summary}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar px-4 py-4 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="bg-zinc-900/80 border border-zinc-800 rounded-xl p-3 flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-[11px] text-zinc-500 uppercase tracking-wide font-medium">Duration</p>
                                <p className="text-white font-semibold text-sm">{generatedPlan.estimatedDuration || '~60 min'}</p>
                            </div>
                        </div>
                        <div className="bg-zinc-900/80 border border-zinc-800 rounded-xl p-3 flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-[11px] text-zinc-500 uppercase tracking-wide font-medium">Exercises</p>
                                <p className="text-white font-semibold text-sm">{generatedPlan.exercises.length} exercises</p>
                            </div>
                        </div>
                    </div>
                    ...
                    {generatedPlan.coachTip && (
                        <div className="bg-gradient-to-r from-violet-500/10 to-blue-500/10 border border-violet-500/20 rounded-xl p-4 flex gap-3">
                            ...
                            <div>
                                <p className="text-xs font-semibold text-violet-300 uppercase tracking-wide mb-1">Coach Tip</p>
                                <p className="text-sm text-zinc-300 leading-relaxed">{generatedPlan.coachTip}</p>
                            </div>
                        </div>
                    )}
                    ...
                    <div>
                        ...
                        >
                            <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider">Workout Plan</h3>
                            {showExerciseDetails ? <ChevronUp className="w-4 h-4 text-zinc-500" /> : <ChevronDown className="w-4 h-4 text-zinc-500" />}
                        </button>
                        ...
                        {showExerciseDetails && (
                            <div className="space-y-2">
                                {generatedPlan.exercises.map((ex, i) => (
                                    <div
                                        key={i}
                                        className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-4 flex items-start gap-4 group hover:border-zinc-700 transition-colors"
                                    >
                                        <div className="w-8 h-8 rounded-lg bg-zinc-800 flex items-center justify-center shrink-0 text-sm font-bold text-zinc-400 group-hover:bg-blue-500/20 group-hover:text-blue-400 transition-colors">
                                            {i + 1}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <h4 className="text-white font-medium text-sm">{ex.name}</h4>
                                            <div className="flex items-center gap-3 mt-1.5 text-xs text-zinc-400">
                                                <span className="bg-zinc-800 px-2 py-0.5 rounded">{ex.sets} sets</span>
                                                <span className="bg-zinc-800 px-2 py-0.5 rounded">{ex.reps} reps</span>
                                                {ex.restSeconds && (
                                                    <span className="bg-zinc-800 px-2 py-0.5 rounded">{ex.restSeconds}s rest</span>
                                                )}
                                            </div>
                                            {ex.notes && (
                                                <p className="text-xs text-zinc-500 mt-2 italic flex items-start gap-1.5">
                ...
                {/* Footer Actions */}
                <div className="flex-none px-4 py-3 border-t border-zinc-800 bg-slate-900 space-y-2">
                    <Button
    ...
    return (
        <div className="fixed top-[56px] left-0 right-0 bottom-0 z-40 bg-slate-900 flex flex-col animate-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex-none px-4 py-4 border-b border-zinc-800/50">
                ...
                {/* Mode Tabs */}
                <div className="flex mt-4 p-1 bg-zinc-900/80 rounded-xl gap-1 border border-zinc-800">
                    <button
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar px-4 py-4 space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {GOALS.map(g => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <Icon className={`w-5 h-5 mb-2 ${isActive ? 'text-violet-400' : 'text-zinc-500'}`} />
                                            <p className={`font-medium text-sm ${isActive ? 'text-white' : 'text-zinc-300'}`}>{g.label}</p>
                            ...
                            <div className="grid grid-cols-4 gap-2">
                                {DURATIONS.map(d => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <p className={`font-bold text-sm ${isActive ? 'text-white' : 'text-zinc-300'}`}>{d.label}</p>
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                {EQUIPMENT.map(e => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <p className={`font-medium text-sm ${isActive ? 'text-white' : 'text-zinc-300'}`}>{e.label}</p>
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <textarea
                                ...
                                className="w-full h-40 bg-zinc-900 border border-zinc-800 rounded-xl p-4 text-white text-sm placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:border-violet-500/50 transition-all resize-none"
                                autoFocus
                            />
                            ...
                            <div className="flex flex-wrap gap-2">
                                {[
                                    ...
                                    <button
                                        ...
                                        className="text-xs px-3 py-1.5 bg-zinc-800 border border-zinc-700 rounded-full text-zinc-400 hover:text-white hover:bg-zinc-700 hover:border-zinc-600 transition-colors"
                                    >
                                        {prompt}
                                    </button>
                                ))}
                ...
                {!isPremium && !subLoading && (
                    <div className="text-xs text-zinc-500 text-center px-2 py-3 bg-zinc-900/50 rounded-xl border border-zinc-800">
            ...
            {/* Footer */}
            <div className="flex-none px-4 py-3 border-t border-zinc-800 bg-slate-900">
                <Button
