        ...
        <Card className="border-zinc-800 bg-zinc-900/50 h-full flex flex-col">
            <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-sm font-medium text-zinc-300 min-w-[140px] text-center">{monthName}</span>
                        <button
                            onClick={nextMonth}
            ...
            <CardContent className="flex-1 flex flex-col">
                ...
                <div className="flex items-center gap-4 mb-4 text-xs text-zinc-400">
                    ...
                    <span className="bg-zinc-800 px-2.5 py-1 rounded-md">
                        <span className="text-white font-semibold">{uniqueDays.size}</span> active days
                    </span>
                </div>
                        ...
                        return (
                            <button
                                ...
                                className={`aspect-square rounded-lg flex flex-col items-center justify-center relative transition-all text-xs
                                    ...
                                    ${hasWorkout ? 'cursor-pointer hover:bg-zinc-800' : 'cursor-default'}
                                    ${isSelected ? 'bg-zinc-800 ring-1 ring-zinc-600' : ''}
                                `}
                            >
                                <span className={`text-[11px] font-medium ${
                                    cell.isToday ? 'text-blue-400' :
                                    hasWorkout ? 'text-white' : 'text-zinc-600'
                                }`}>
                                    {cell.day}
                                </span>
                                {hasWorkout && (
                ...
                {selectedDay && workoutMap[selectedDay] && (
                    <div className="mt-3 p-3 bg-zinc-900/80 border border-zinc-800 rounded-xl space-y-2 animate-in slide-in-from-top-2 duration-200">
                        ...
                        {workoutMap[selectedDay].map((w, i) => (
                            <div key={i} className="flex items-center gap-2">
                                ...
                                <span className="text-sm text-white font-medium">{w.type}</span>
                ...
                <div className="flex flex-wrap gap-2 mt-auto pt-3 border-t border-zinc-800 justify-center">
                    {[
                        ...
                        <div key={item.label} className="flex items-center gap-1 text-[10px] text-zinc-500">
